...
export default class Collapsible extends Component<PropsWithChildren<ICollapsibleProps>, ICollapsibleState> {
  ...
  _animation: Animated.CompositeAnimation | void | null = null;
  ...
  componentDidUpdate(prevProps: ICollapsibleProps) {
  ...
  _componentDidUpdate(prevProps: ICollapsibleProps) {
    if (prevProps.collapsed !== this.props.collapsed) {
  ...
  _measureContent(callback: (ht: number) => void) {
    this.setState(
      {
        measuring: true,
      },
  ...
  render() {
    ...
    const contentStyle: Animated.AnimatedProps<StyleProp<ViewStyle>> = {};
    if (measuring) {
    ...
    const shouldRenderChildren =
      renderChildrenCollapsed || ((!collapsed || (collapsed && animating)) && (animating || measuring || measured));

    return (
      <Animated.View style={style as any} pointerEvents={!enablePointerEvents && collapsed ? "none" : "auto"}>
        <Animated.View
          ref={this._handleRef}
          style={[this.props.style as any, contentStyle]}
